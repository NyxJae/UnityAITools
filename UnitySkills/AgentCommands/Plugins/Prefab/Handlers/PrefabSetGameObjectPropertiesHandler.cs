...
namespace AgentCommands.Plugins.Prefab.Handlers
{
    ...
    internal static class PrefabSetGameObjectPropertiesHandler
    {
        ...
        public static JsonData Execute(JsonData rawParams)
        {
            ...
            {
                throw new ArgumentException(AgentCommandErrorCodes.InvalidFields + ": properties is required");
            }

            JsonData properties = parameters.GetData()["properties"];
            if (properties == null || !properties.IsObject || properties.Count == 0)
            {
                throw new ArgumentException(AgentCommandErrorCodes.InvalidFields + ": properties对象不能为空, 至少需要指定一个要修改的属性");
            }
