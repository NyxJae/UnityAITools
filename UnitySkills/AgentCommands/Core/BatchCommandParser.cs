...
namespace AgentCommands.Core
{
    ...
    internal static class BatchCommandParser
    {
        ...
        public static BatchPendingCommand Parse(string filePath, DateTime fileTime)
        {
            ...
            {
                throw new ArgumentException(AgentCommandErrorCodes.InvalidJson + ": 文件不存在: " + filePath);
            }
            ...
            catch (Exception ex)
            {
                ...
                throw new ArgumentException(AgentCommandErrorCodes.InvalidJson + ": JSON解析失败 - " + ex.Message);
            }

            if (root == null || !root.IsObject)
            {
                throw new ArgumentException(AgentCommandErrorCodes.InvalidJson + ": 批量命令文件不是合法的json,请检查写入是否完整");
            }
        ...
        public static void Validate(BatchPendingCommand batchCmd)
        {
            if (batchCmd == null)
            {
                throw new ArgumentException(AgentCommandErrorCodes.InvalidFields + ": 批量命令为null");
            }

            if (string.IsNullOrEmpty(batchCmd.batchId))
            {
                throw new ArgumentException(
                    AgentCommandErrorCodes.InvalidFields + ": 缺少必填字段: batchId - 输入 JSON 必须包含 batchId 字段");
            }
            ...
            {
                throw new ArgumentException(
                    AgentCommandErrorCodes.InvalidFields + ": batchId包含非法字符,仅允许字母数字下划线和短横线 - batchId=" + batchCmd.batchId);
            }

            if (batchCmd.commands == null || batchCmd.commands.Count == 0)
            {
                throw new ArgumentException(
                    AgentCommandErrorCodes.InvalidFields + ": 命令数组不能为空 - commands 数组至少包含一个命令");
            }
            ...
            for (int i = 0; i < batchCmd.commands.Count; i++)
            {
                ...
                {
                    throw new ArgumentException(
                        AgentCommandErrorCodes.InvalidFields + $": 命令缺少必填字段: id - 第 {i + 1} 个命令必须包含 id 字段");
                }

                if (string.IsNullOrEmpty(cmd.type))
                {
                    throw new ArgumentException(
                        AgentCommandErrorCodes.InvalidFields + $": 命令缺少必填字段: type - 命令 id={cmd.id} 必须包含 type 字段");
                }

                if (cmd.@params == null)
                {
                    throw new ArgumentException(
                        AgentCommandErrorCodes.InvalidFields + $": 命令缺少必填字段: params - 命令 id={cmd.id} 必须包含 params 字段");
                }
            }
        }
