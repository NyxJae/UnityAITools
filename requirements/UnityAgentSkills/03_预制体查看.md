...
- 定义: GameObject 在父对象的 `transform.GetChild(i)` 中的索引 `i`
- 范围: 从 0 开始 (第一个子对象为 0,第二个为 1,以此类推)
- 跨会话稳定: 索引基于层级结构,不随 Unity Editor 重启变化
- 用途: 精确定位同路径下的同名对象
...
private static HierarchyNode TraverseRecursive(GameObject obj, ...)
{
    var node = new HierarchyNode
    {
        ...
        siblingIndex = obj.transform.GetSiblingIndex(),  // 新增
        depth = depth,
...
Panel_Content/
  ...
  ├─ K3Label_Title     (siblingIndex=1)
  ├─ K3Button_Confirm  (siblingIndex=2) ← 匹配目标
  └─ K3Button_Cancel   (siblingIndex=3)
```
...
| prefabPath           | string   | 是   | -      | 预制体绝对路径                           |
| objectPath           | string   | 是   | -      | GameObject 层级路径                      |
| siblingIndex         | int      | 否   | 0      | 同级中的索引(从 0 开始),用于定位同名对象 |
| componentFilter      | string[] | 否   | null   | 组件类型过滤,null 表示全部               |
| includeBuiltin       | boolean  | 否   | false  | 是否包含 Unity 内置组件                  |
| includePrivateFields | boolean  | 否   | false  | 是否包含私有字段                         |
