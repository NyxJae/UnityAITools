...
- **插件化架构**: prefab 命令通过 `UnityAgentSkills.Plugins.Prefab.PrefabCommandsPlugin` 提供(Priority 20)
- **实现文件**: `Code/Assets/Editor/UnityAgentSkills/Plugins/Prefab/PrefabCommandsPlugin.cs`
...
- [ ] 当传入 `nameContains` 时,不返回完整 hierarchy 树,改为返回扁平列表 `matches[]`(每项含 path 和基础信息)
...
- 此时 `result` 仍保留 `prefabPath` 和 `rootName`,并新增:
  ...
  - `matchedCount`: 本次实际返回的 matches 条数(<= maxMatches)
  - `truncated`: 当 totalMatches > matchedCount 时为 true
- 当匹配数量超过 `maxMatches` 时,仅返回前 `maxMatches` 条.
...
| nameContains    | string  | 否   | null   | 名称模糊过滤(contains),IgnoreCase,对入参 Trim.当传入时返回 `matches[]`,不返回 `hierarchy`.                                        |
| maxMatches      | int     | 否   | 50     | 当传入 nameContains 时,最多返回的匹配条数(防止结果过大).超过时截断,并通过 `totalMatches`,`matchedCount`,`truncated` 反映截断情况. |
...
#### GameObjectPathFinder.FindByPath 增强逻辑
...
- **非最后一段**: 名称匹配即可,不检查 `siblingIndex` (继续向下遍历)
- **最后一段**: 名称匹配 + `siblingIndex` 匹配 (精确定位目标)
- **默认值**: `siblingIndex = 0` 确保向后兼容

#### HierarchyTraverser.Traverse 增加 siblingIndex 记录

```csharp
// 伪代码示意
private static HierarchyNode TraverseRecursive(GameObject obj, ...)
{
    var node = new HierarchyNode
    {
        name = obj.name,
        path = GameObjectPathFinder.GetPath(obj),
        siblingIndex = GameObjectPathFinder.GetSameNameSiblingIndex(obj),  // 新增:同名对象序号
        depth = depth,
