...
- `componentFilter: ["K3Button", "transform"]` 使用模糊匹配
- `"transform"` 会匹配所有名称包含 `transform` 的组件,如 `Transform`, `RectTransform`.
- `"K3Button"` 会匹配所有名称包含 `K3Button` 的组件,如 `K3Button`, `K3ButtonGroup`.
- 本示例仅用于演示匹配规则,示例结果假设目标对象当前只挂载了 `Transform` 和 `K3Button` 两个组件.
...
```json
{
  "params": {
    "prefabPath": "Assets/resourcesab/uiprefabs/DialogMain.prefab",
    "objectPath": "DialogMain/Panel_Content/K3Button_Confirm",
    "componentFilter": ["K3Button", "k3"]
  }
}
```

预期返回：返回所有名称包含 "K3Button" 或 "k3" 的组件,如 `K3Button`, `K3ButtonGroup`, `K3Label` 等
...
预期返回：返回所有组件,包括 `Transform`, `RectTransform`, `CanvasRenderer`, `K3Button` 等所有组件
...
当预制体中存在同路径同名称的多个 GameObject 时(例如 Panel_Content 下有多个 K3Button_Confirm)
...
DialogMain/
  └─ Panel_Content/
      ├─ K3Button_Confirm  (第一个, siblingIndex=0)
      ├─ K3Button_Confirm  (第二个, siblingIndex=1)
      └─ K3Button_Cancel
```
...
```json
{
  ...
  "params": {
    "prefabPath": "Assets/.../DialogMain.prefab",
    "objectPath": "DialogMain/Panel_Content/K3Button_Confirm",
    "siblingIndex": 1 // 查询第二个 K3Button_Confirm
  }
}
```
...
```
Panel_Content/
  ├─ K3Button_Confirm  (siblingIndex=0) ← 跳过
  ├─ K3Label_Title     (不同名,不参与 K3Button_Confirm 的 siblingIndex 计数)
  ├─ K3Button_Confirm  (siblingIndex=1) ← 匹配目标
  └─ K3Button_Cancel   (不同名,不参与 K3Button_Confirm 的 siblingIndex 计数)
```

**结果**: 返回第二个 `K3Button_Confirm` 的组件信息
...
3. **后续操作** (查询组件或编辑):
   ...
   prefab.queryComponents(
     "DialogMain.prefab",
     "DialogMain/Panel_Content/K3Button_Confirm",
