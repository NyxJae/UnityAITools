...
需要提供专门的命令来查询和编辑 K3 框架组件，同时集成通用的预制体查看与编辑能力：

**K3 框架特有命令：**

1. **k3prefab.queryByK3Id**: 通过 K3ID 查询 K3 组件信息
2. **k3prefab.setComponentProperties**: 修改 K3 组件的属性参数
3. **k3prefab.createComponent**: 创建新的 K3UI 组件

**集成的通用预制体命令（来自 unity-prefab-view）：** 4. **prefab.queryHierarchy**: 查询预制体层级结构 5. **prefab.queryComponents**: 查询指定 GameObject 的组件详情 6. **prefab.setGameObjectProperties**: 修改 GameObject 通用属性 7. **prefab.deleteGameObject**: 删除 GameObject 8. **prefab.moveOrCopyGameObject**: 移动或复制 GameObject

### 1.2 项目现状

- 项目已有完善的 UnityAgentSkills 命令系统，位于`Code/Assets/Editor/UnityAgentSkills/`
- 前置能力(依赖): 本功能依赖预制体查看与编辑能力,需先具备 `prefab.queryHierarchy`,`prefab.queryComponents`,`prefab.setGameObjectProperties`
...
### 1.2.1 Phase1 架构约束(协议一致性)

- **prefabPath 校验对齐**: K3Prefab 与通用 Prefab 共用 `PrefabComponentHandlerUtils.ValidatePrefabPathOrThrow`，非法路径统一返回 `INVALID_FIELDS`。
- **错误码白名单**: K3 特有错误码纳入框架白名单，避免降级为 `RUNTIME_ERROR`:
  - `EMPTY_MODIFICATIONS`: modifications 数组为空或缺失
  - `ID_NOT_FOUND`: 未找到指定 ID（原为 K3ID_NOT_FOUND，已泛化）
  - `INDEX_OUT_OF_RANGE`: 索引超出范围
- **校验复用**: K3Prefab Handler 复用通用 Prefab 的参数校验工具（siblingIndex/componentIndex/properties 空校验等），减少协议漂移。
...
#### 命令 3: k3prefab.createComponent

- [ ] 接收预制体路径、父节点路径、组件类型、初始属性（可选）
- [ ] 在指定的 K3Dialog/K3Panel 下创建新的 K3UI 组件
- [ ] 自动分配 K3ID 并设置 parentID
- [ ] 支持 20+ 种 K3 组件类型（K3Button, K3Label, K3Image 等）
- [ ] 返回创建的 GameObject 路径和 K3 组件信息

#### 命令 4: prefab.queryHierarchy (集成自 unity-prefab-view)

- [ ] 接收预制体路径，返回完整层级结构
- [ ] 支持 `nameContains` 参数进行名称模糊过滤
- [ ] 支持 `includeInactive` 控制是否包含禁用节点
- [ ] 支持 `maxDepth` 限制遍历深度
- [ ] 默认返回嵌套树结构，过滤模式返回扁平 `matches[]` 列表

#### 命令 5: prefab.queryComponents (集成自 unity-prefab-view)

- [ ] 接收预制体路径和 GameObject 路径，返回组件详情
- [ ] 支持 `siblingIndex` 定位同名对象
- [ ] 支持 `componentFilter` 按类型过滤组件
- [ ] 支持 `includeBuiltin` 控制是否包含 Unity 内置组件
- [ ] 返回组件类型、instanceID 和可序列化属性

#### 命令 6: prefab.setGameObjectProperties (集成自 unity-prefab-view)

- [ ] 接收预制体路径、objectPath、要修改的属性对象
- [ ] 支持修改 name, tag, layer, isActive 等 GameObject 通用属性
- [ ] 支持 `siblingIndex` 定位同名对象
- [ ] 返回修改前后的属性值对比

#### 命令 7: prefab.deleteGameObject (集成自 unity-prefab-view)

- [ ] 接收预制体路径和 objectPath，删除指定 GameObject
- [ ] 支持 `siblingIndex` 定位同名对象
- [ ] 级联删除所有子物体
- [ ] 禁止删除预制体根节点

#### 命令 8: prefab.moveOrCopyGameObject (集成自 unity-prefab-view)

- [ ] 接收源路径和目标父路径，移动或复制 GameObject
- [ ] 通过 `isCopy` 参数控制操作类型
- [ ] 支持 `sourceSiblingIndex` 和 `targetSiblingIndex` 定位
- [ ] 自动保持世界坐标不变

### 2.2 排除项

- 不支持通过 GameObject 路径直接查询 K3 组件（使用 `prefab.queryComponents` 查询组件后自行过滤）
- 不支持修改非 K3 框架的 Unity 组件属性
- 不支持删除 K3 组件（仅支持删除整个 GameObject）
- 不支持修改运行时实例（仅支持预制体文件）
- 不支持跨预制体的组件复制
...
- 整个`K3Prefab`文件夹可以独立删除，不影响其他 UnityAgentSkills 功能
- 不依赖 K3 框架之外的特定项目逻辑
- 清晰的模块边界，便于后续移植或移除
...
- [ ] 新建`Assets/Editor/UnityAgentSkills/K3Prefab/`文件夹
- [ ] 所有 K3 相关代码都在该文件夹中
- [ ] 整个文件夹可以独立删除，不影响其他 UnityAgentSkills 功能
- [ ] 不依赖 K3 框架之外的特定项目逻辑
...
unity-k3-prefab 技能需要集成以下所有命令:

**K3 框架特有命令:**

- `k3prefab.queryByK3Id` - 通过 K3ID 查询 K3 组件信息
- `k3prefab.setComponentProperties` - 修改 K3 组件属性
- `k3prefab.createComponent` - 创建新的 K3UI 组件

**unity-prefab-view 的全部命令(需完整集成):**

- `prefab.queryHierarchy` - 查询预制体层级结构(支持 nameContains 过滤模式)
- `prefab.queryComponents` - 查询指定 GameObject 的组件详情

**其他通用的 GameObject 编辑命令(尚未做成 skill)**

- `prefab.deleteGameObject` - 删除 GameObject
- `prefab.moveOrCopyGameObject` - 移动或复制 GameObject

参考 requirements/UnityAgentSkills/03*预制体查看.md 获取 prefab.\* 命令的完整协议定义.
参考 requirements/UnityAgentSkills/04*预制体编辑 01.md 获取编辑类命令的定义.

**SKILL.md 文档精简要求:**

由于集成命令较多,生成的 SKILL.md 文档会较长,需要对返回结果示例进行精简:

1. **保留一个全量返回示例** - 选择 `k3prefab.queryByK3Id` 或 `prefab.queryHierarchy` 作为全量示例,展示完整的返回结构
2. **其余命令只保留参数说明** - 其他命令仅提供:
   - 命令用途简述
   - 参数列表及说明
   - 简单的返回字段说明(不包含完整 JSON 示例)
   - 单命令调用示例(bash 命令行形式)

**示例精简对比:**

- 全量示例(保留): 包含完整的 JSON 输入输出、字段解释、边缘情况
- 精简示例(其他命令): 只保留参数表格和调用示例,如:
...
- `siblingIndex` 可选,同名对象索引,默认 0

**调用示例**:

```bash
uv run "<Scripts Directory>/execute_unity_command.py" '{"batchId":"...","commands":[{"type":"prefab.deleteGameObject","params":{...}}]}'
```
````

```

参考 F:\\UnityProject\\SL\\SL_402\\Code\\Assets\\Editor\\UnityAgentSkills\\SkillsExporter
requirements/UnityAgentSkills/99_用户界面插件与skill和脚本生成.md
制作新的 k3-prefab 技能
```
