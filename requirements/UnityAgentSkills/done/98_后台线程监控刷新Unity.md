...
本需求不约束实现类名,文件名,或代码组织结构.

对实现的硬约束仅包括:

- Unity 编辑器启动时,服务必须能自动初始化并进入可工作状态.
- 服务必须能按配置启用/禁用,并支持用户在 UI 中修改配置.
- 配置必须按 Unity 项目维度持久化(不同项目互不干扰).
- Play Mode 期间必须按需求中的规则暂停/恢复.
