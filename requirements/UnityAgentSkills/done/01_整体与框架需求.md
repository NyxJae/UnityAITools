...
- 关键路径约定:
  - 数据目录: `Assets/UnityAgentSkills/Commands/`.

> 说明: 本需求只约束命令数据目录(pending/results/done)的对外协议,不约束插件代码放置位置.

## 0.1 架构约束摘要

- 框架采用两层隔离架构:
  - 内置层(priority 0,硬编码注册,不依赖反射): 命名空间为 `log.*`,至少包含 `log.query`,`log.screenshot`,`log.refresh`.
  - 反射插件层: `prefab.*` 等.
- Log 插件层的特殊约束:
...
- **错误码白名单机制(强约束)**:
  - Handler 抛异常时格式必须为 `CODE: message`(兼容 `CODE:message` 和 `CODE:` 两种变体).
  - 框架通过“错误码白名单”识别错误码,白名单内的错误码直接透传,不降级为 `RUNTIME_ERROR`.
  - 新增错误码时,必须同步更新:
    - 错误码常量真源.
    - 错误码白名单真源.
    - 对外导出文档真源(例如技能文档中的错误码清单).
- **prefabPath 统一校验规则**:
  - 必须满足 `Assets/.../*.prefab` 格式,否则返回 `INVALID_FIELDS`.

---

## 1. 项目现状与核心目标

本需求用于实现一个 Unity Editor 插件(编辑器侧常驻工具),在 Unity 启动后自动运行,监听项目内 `Assets/UnityAgentSkills/Commands/pending/` 目录新增的 `.json` 命令文件,并以"先写处理中结果,再写最终结果,最后归档命令文件"的方式可靠地完成命令处理.
...
- `log.screenshot` - Unity Game 视图截图
- `log.refresh` - 手动触发 Unity 刷新并等待刷新/编译完成
- `editor.runAction` - 运行编辑器动作(菜单/按钮/快捷键/插件公开方法)
- `prefab.queryHierarchy` - 预制体层级查询
- `prefab.queryComponents` - 预制体组件查询
- `prefab.setGameObjectProperties` - GameObject 属性修改
- `prefab.setComponentProperties` - 组件属性修改
...
- 路径协议: 插件仅监听 `Assets/UnityAgentSkills/Commands/pending/`.
...
### 2.1 插件化架构与代码组织(非目录约束)

本需求关注"命令处理协议"与"结果语义",而非插件代码放置位置.

对实现的约束仅包括:

- 内置层(`log.*`)与反射插件层必须隔离,且注册优先级与故障隔离规则明确(见后文 2.2).
- 命令处理框架必须通过"pending -> results -> done"的文件闭环对外提供稳定协议(见后文 2.3 及第 3/4 节).

### 2.2 两层隔离架构

**核心理念**: 日志命令(log._)属于内置层,硬编码注册不依赖反射. 当前内置命令至少包含: log.query, log.screenshot, log.refresh. 其他所有命令(包括 Unity 通用的 prefab._)都通过反射插件机制加载.

**⚠️ 重要说明**:

- log.query, log.screenshot, log.refresh 是内置命令,硬编码注册在 `CoreCommandsLoader` 中,priority 0,永不依赖反射
...
- 所有其他命令(prefab.\_,未来新增的 asset.\*)都通过反射插件机制加载
...
**命令数据根目录(必须):** `Assets/UnityAgentSkills/Commands/`

**数据子目录(若不存在必须自动创建):**

- `Assets/UnityAgentSkills/Commands/pending/`
- `Assets/UnityAgentSkills/Commands/results/`
- `Assets/UnityAgentSkills/Commands/done/`

**功能点(本期必须):**

- [ ] Unity Editor 启动后自动启动插件,并开始监听 `Assets/UnityAgentSkills/Commands/pending/`.
- [ ] Unity Editor 本次会话首次初始化时,先清理 `Assets/UnityAgentSkills/Commands/pending/` 中遗留且未处理完成的 json,避免上次卡住命令被意外执行.
- [ ] 脚本重编译触发域重载后,不得清理 `Assets/UnityAgentSkills/Commands/pending/`,必须扫描并继续处理未完成的 json.
- [ ] 提供用户可手动触发的“重启命令服务并清理 pending”能力,用于服务卡住时应急恢复(清理目标为 `Assets/UnityAgentSkills/Commands/pending/`).
- [ ] 发现 `Assets/UnityAgentSkills/Commands/pending/` 中存在待处理 json 时,按“单线程串行”策略处理(同时最多 1 个命令).
- [ ] 每个命令开始处理时,先在 `Assets/UnityAgentSkills/Commands/results/` 写入“半成品(处理中)json”作为处理中的标识.
- [ ] 处理完成后,覆盖写入 `Assets/UnityAgentSkills/Commands/results/` 中对应 json 为最终结果.
- [ ] 处理成功或失败后,将 `Assets/UnityAgentSkills/Commands/pending/` 中对应 json 移动到 `Assets/UnityAgentSkills/Commands/done/` 归档.
...
- [ ] 稳定性: 即使处理中崩溃/重启,也能通过 `Assets/UnityAgentSkills/Commands/pending/` + `Assets/UnityAgentSkills/Commands/results/` 状态恢复并继续.
- [ ] 可审计: `Assets/UnityAgentSkills/Commands/results/` 中结果必须包含清晰的状态与时间信息(便于排查).
...
- 输入文件必须放在 `Assets/UnityAgentSkills/Commands/pending/`.
- 后缀必须是 `.json`.

**文件命名规则(必须):**

- 文件名必须为 `{batchId}.json`,其中 `batchId` 即文件名去掉 `.json` 的部分.
- json 内容必须提供 `batchId` 字段,且必须与文件名中的 `{batchId}` 一致.
...
- 推荐长度: 16-32 个字符.
- 示例: `batch_log_001`, `batch_error_qa2uim9x`.
...
- `type`: string,必填.命令类型,用于分发处理器.至少包含 `log.query`,`editor.runAction`,具体以 1.1 命令清单为准.
...
**文件位置:** `Assets/UnityAgentSkills/Commands/results/`.
...
- `Assets/UnityAgentSkills/Commands/results/` 中最多保留最近 20 条最终结果(success/error). 当写入新的最终结果后,如果超出 20 条,必须删除最老的结果文件.

  - 同时删除 `Assets/UnityAgentSkills/Commands/done/` 中同名归档命令文件(即原始 pending json 的备份).
  ...
  - 涉及 json 文件的归档与清理时,必须将“json 本体 + 同路径同文件名追加 `.meta` 得到的 sidecar 文件”视为同一组成功条件.
  - 归档 `Commands/pending/{batchId}.json -> Commands/done/{batchId}.json` 时,只有源 json 与源 `.meta` 都存在且都成功移动到目标位置,才算归档成功.

- 清理旧的 `Commands/results/{batchId}.json` 与 `Commands/done/{batchId}.json` 时,只有 json 与对应 `.meta` 都存在且都成功删除,才算清理成功.

  - 若 json 本体存在但 `.meta` 不存在,或联动移动/删除任一环节失败,则本次动作必须静默跳过或静默放弃,不输出 warning,也不得把半套状态认定为成功.
  - 不要求补写,重建,或自动重新导入以恢复缺失的 `.meta`.

- 在域重载后重新启动插件时,如果发现 `Assets/UnityAgentSkills/Commands/pending/` 仍存在同一个命令文件,则允许覆盖 `results/{id}.json` 为新的 `processing`(表示继续处理).
...
- 单线程串行处理.一次只处理一个批次文件.
- 当 `Assets/UnityAgentSkills/Commands/pending/` 同时存在多个文件,按"文件创建时间从早到晚"处理(若无法可靠获取,则按文件名排序作为兜底,实现阶段需注明).
- 批次内的命令严格按照 `commands` 数组的顺序串行执行,不并行.

**处理流程(必须):**

1. 发现待处理 `Assets/UnityAgentSkills/Commands/pending/{batchId}.json`.
2. 读取并解析 json.
3. 若 json 不可解析,或缺少最小字段(例如缺 `batchId`/`commands`):
   - 直接写入 `Commands/results/{batchId}.json` 为 `status=error`(不写 processing),用于告知外部"命令文件格式不正确\".
   - `error.code` 使用 `INVALID_JSON` 或 `INVALID_FIELDS`.
   - 然后将 `Commands/pending/{batchId}.json` 移动到 `Commands/done/{batchId}.json` 归档.
4. 若校验通过,写入 `Commands/results/{batchId}.json` 半成品: `status=processing`, `results=[]`.
5. 串行执行批次内的每个命令:
...
6. 所有命令执行完毕或批次超时后:
   - 覆盖写入 `Commands/results/{batchId}.json` 为 `status=completed`,包含完整的 `results` 数组和统计信息(`totalCommands`,`successCount`,`failedCount`).
7. 无论成功或失败,将 `Commands/pending/{batchId}.json` 移动到 `Commands/done/{batchId}.json`.
   - 若 `Commands/pending/{batchId}.json.meta` 不存在,则本次归档静默放弃,不只移动 json 本体,也不输出 warning.
   - 若 json 与 `.meta` 都存在,则必须将两者作为同一组成功条件一起移动到 `Assets/UnityAgentSkills/Commands/done/`.
   - 只要任一移动环节失败,都不得把这次归档视为成功,且不额外制造用户可见 warning.

**恢复机制(必须):**

- Unity Editor 启动后插件首次初始化时:
  - 必须先清理 `Assets/UnityAgentSkills/Commands/pending/` 中上次会话遗留且未处理完成的 json,避免卡住命令跨会话误执行.
- Unity Editor 本次会话内,脚本重编译触发域重载后插件重新加载时:
  - 不得清理 `Assets/UnityAgentSkills/Commands/pending/`.
  - 必须扫描 `Assets/UnityAgentSkills/Commands/pending/`,继续处理未归档文件.
  - 即使 `results/{batchId}.json` 已存在并且是 `processing`,也必须以当前重启为准继续处理,并可覆盖更新 `startedAt`.
- 提供手动应急能力:
  - 用户可通过菜单触发“重启命令服务并清理 pending”,用于服务卡住时快速恢复.
...
本需求不约束插件代码组织形式,但对插件对外行为有以下最小要求:

- 每个插件必须有一个稳定的插件名(Name),用于日志与调试定位.
- 每个插件必须能向框架注册其支持的命令(type -> handler 映射).
- 插件必须支持可选的初始化与关闭(用于分配/释放资源).
- 插件必须能被框架自动发现(例如通过某种声明性标记或注册机制).该标记至少包含:
  - 插件名: 必填.
  - 插件优先级(Priority): 可选.数值越小,越先加载.默认值为 100.
...
- 每个插件必须在独立的错误隔离边界中加载.
- 单个插件加载失败时,只记录错误并跳过该插件,不得影响其他插件与内置命令可用性.
- 框架应记录每个插件实际注册的命令清单与优先级,便于排查命令覆盖与冲突.
...
- 即使所有插件都加载失败,log.query 仍可用(同时 log.screenshot 和 log.refresh 也应可用)
...
- 删除项目专用插件目录后项目可正常编译
- `CommandHandlerRegistry` 不再直接引用项目专用处理器命名空间
- 所有项目专用引用通过接口解耦
...
- 场景:
  - 外部程序正在写 `Commands/pending/cmd_002.json`,Unity 监听到 Created/Changed 事件时文件内容还不完整.
- 预期:
...
- 场景:
  - 已写出 `Commands/results/cmd_003.json` 为 `processing`,但 Unity 因脚本重编译域重载.
- 预期:
  - 重启后扫描 `Assets/UnityAgentSkills/Commands/pending/` 仍发现 `cmd_003.json`,继续处理并最终归档.
...
- [ ] 创建 `CoreCommandsLoader` 类,硬编码注册核心命令(至少 log.query,log.screenshot,log.refresh)
...
- [ ] 往 `Assets/UnityAgentSkills/Commands/pending/` 放入 1 个合法批量命令 json,json 会被处理,`Assets/UnityAgentSkills/Commands/results/` 先出现 processing 再变 completed,命令文件被移动到 `Assets/UnityAgentSkills/Commands/done/`.
...
- [ ] 在处理过程中检查 `Assets/UnityAgentSkills/Commands/results/` 目录,即使已有 20 条或更多结果,`processing` 状态的结果文件也不会被清理;只有 `completed` 的最终结果才参与计数和清理.

### 8.6 可移植性验收

- [ ] 若存在任何项目专用插件实现,移除该实现后,项目编译无错误
- [ ] 若存在任何项目专用插件实现,移除该实现后,Unity 启动无错误
- [ ] 若存在任何项目专用插件实现,移除该实现后,其他命令功能正常
...
- `02_日志功能需求_log.md`: 日志相关内置命令(`log.query`,`log.screenshot`,`log.refresh`)的需求与验收口径.
- `03_预制体查看.md`: 预制体查看命令(`prefab.queryHierarchy`,`prefab.queryComponents`)的需求与验收口径.
- `04_预制体编辑.md`: 预制体编辑类命令的最终需求与验收口径.
- `99_用户界面插件与skill和脚本生成.md`: SkillsExporter 的需求与约束.导出产物为统一的 `unity-agent-skills` 技能包.
