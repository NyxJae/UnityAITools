...
- `nameContains`: 可选,`string[]`,按对象名 contains 匹配(IgnoreCase),多项 OR.对每个词项先做 Trim,空数组,或仅包含空字符串时视为不传.
- `textContains`: 可选,`string[]`,按文本 contains 匹配(IgnoreCase),多项 OR.对每个词项先做 Trim,空数组,或仅包含空字符串时视为不传.
- `componentFilter`: 可选,组件类型 contains + IgnoreCase + OR 匹配.对每个词项先做 Trim,空数组,或仅包含空字符串时视为不传.
...
- 除布尔开关外,文本类过滤参数在传入有效值但没有命中时,正常返回空结果,不报错
...
```json
{
  ...
  "results": [
    ...
    {"id": "3", "status": "success", "result": {"mode": "single", "imageAbsolutePath": ".../Assets/UnityAgentSkills/Commands/results/test_newbie_001_3.png", "highlightApplied": true}},
    {"id": "4", "status": "success"},
    ...
    {"id": "10", "status": "success", "result": {"mode": "single", "imageAbsolutePath": ".../Assets/UnityAgentSkills/Commands/results/test_newbie_001_10.png", "highlightApplied": false}},
...
| `nameContains`    | contains + IgnoreCase + OR           | hierarchy 搜索相同                  |
| `textContains`    | contains + IgnoreCase + OR           | 与日志 `keyword/Fuzzy` 同属文本搜索 |
...
- `nameContains` 看对象名.
- `textContains` 看界面文本.
- `componentFilter` 看组件类型.
- `visibleOnly`,`interactableOnly` 用于收紧结果范围.
- 除布尔开关外,文本类过滤参数都遵循: 不填就是不过滤,contains,IgnoreCase,多项 OR.
- 当传入了有效过滤值但没有任何元素命中时,正常返回空结果,不报错.
...
| 字段               | 类型     | 必填 | 约束                                                     | 非法值示例               |
| ------------------ | -------- | ---- | -------------------------------------------------------- | ------------------------ |
| `nameContains`     | string[] | 否   | 每个词项 Trim 后可为空,空数组/空词项视为未传,多项 OR     | `[123]`                  |
| `textContains`     | string[] | 否   | 每个词项 Trim 后可为空,按文本 contains + IgnoreCase + OR | `[123]`                  |
...
| `nameContains` | string[] | 否   | 每个词项 Trim 后可为空 | 对象名 contains + IgnoreCase,多项 OR          |
| `textContains` | string[] | 否   | 每个词项 Trim 后可为空 | 文本 contains + IgnoreCase,多项 OR            |

**约束处理**:

- `nameContains` 和 `textContains` 为 OR 关系,任一词项命中即停止
