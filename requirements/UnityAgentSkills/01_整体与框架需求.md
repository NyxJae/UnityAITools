...
- **错误码白名单机制(强约束)**:
  ...
  - 框架通过 `IsKnownCommandErrorCode` 白名单识别错误码,白名单内的错误码直接透传,不降级为 `RUNTIME_ERROR`.
  - 新增错误码必须三处同步: `AgentCommandModels.cs`(常量定义) + `UnityAgentSkillsPlugin.cs`(白名单) + `SkillConfig_*`(文档).
- **prefabPath 统一校验规则**:
...
- `prefab.queryByUIId` - 通过 UIID 查询 UI 组件
- `prefab.setComponentProperties` - 修改 UI 组件属性
...
│   └── UIPrefab/                      # UI 框架专用插件 (可移除)
│       ├── UIPrefabPlugin.cs
│       ├── Handlers/
│       │   ├── UIPrefabQueryByUIIdHandler.cs
│       │   └── UIPrefabSetComponentPropertiesHandler.cs
│       ├── Utils/                     # UI 工具类
│       └── Models/                    # UI 数据模型
├── Utils/                             # 通用工具 (保持不变)
...
- [ ] Unity Editor 本次会话首次初始化时,先清理 `pending/` 中遗留且未处理完成的 json,避免上次卡住命令被意外执行.
- [ ] 脚本重编译触发域重载后,不得清理 `pending/`,必须扫描并继续处理未完成的 json.
- [ ] 提供用户可手动触发的“重启命令服务并清理 pending”能力,用于服务卡住时应急恢复.
...
- Unity Editor 启动后插件首次初始化时:
  - 必须先清理 `pending/` 中上次会话遗留且未处理完成的 json,避免卡住命令跨会话误执行.
- Unity Editor 本次会话内,脚本重编译触发域重载后插件重新加载时:
  - 不得清理 `pending/`.
  - 必须扫描 `pending/`,继续处理未归档文件.
...
- [ ] 移除 `UIPrefab/` 文件夹后,项目编译无错误
- [ ] 移除 `UIPrefab/` 文件夹后,Unity 启动无错误
- [ ] 移除 `UIPrefab/` 文件夹后,其他命令功能正常
