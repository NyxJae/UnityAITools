...
**项目已从 AgentCommands 正式改名为 UnityAgentSkills**

- ✅ **代码层**: 所有类名、命名空间、配置键、日志前缀已更新
- ✅ **目录层**: 插件文件夹从 `Assets/Editor/AgentCommands/` 改为 `Assets/Editor/UnityAgentSkills/`
- ✅ **数据层**: 数据目录从 `Assets/AgentCommands/` 改为 `Assets/UnityAgentSkills/`

**主要变更**:

- 核心类名: `AgentCommandsConfig` → `UnityAgentSkillsConfig`, `AgentCommandsPlugin` → `UnityAgentSkillsPlugin`
- 状态/错误码: `AgentCommandStatuses` → `UnityAgentSkillCommandStatuses`, `AgentCommandErrorCodes` → `UnityAgentSkillCommandErrorCodes`
- 路径常量: `DataDirRelativeToAssets` 从 `"AgentCommands"` 改为 `"UnityAgentSkills"`
- 配置键: `AgentCommands.AutoCompile` → `UnityAgentSkills.AutoCompile`

⚠️ **注意**: 正确拼写为 `UnityAgentSkills` (Agent 不是 Agnet)

---

## ⚠️ 架构简化说明 (2026-02-01)

**核心理念**: 只有日志命令 (log.query) 是内置命令,硬编码注册不依赖反射。其他所有命令都通过反射插件机制加载。

**关键决策**:

- ✅ **保留**: log.query 作为唯一内置命令 (Priority 0,硬编码注册)
- ❌ **删除**: system.status 和 plugin.list 命令 (避免维护成本)
- ✅ **所有其他命令**: prefab._, k3prefab._, 未来扩展都通过反射插件加载

**Log 插件的特殊性**:

- LogCommandsPlugin 类存在,但 `RegisterHandlers()` 是**空实现**
- log.query 已在 `CoreCommandsLoader` 中硬编码注册,Priority 0
- 如果在 LogCommandsPlugin.RegisterHandlers 中注册,会使用默认 Priority 100,覆盖内置层
- 保持 LogCommandsPlugin 类是为了代码规范统一,作为插件实现的参考示例

**架构优势**:

- 极简架构: 只有两层 (内置层 + 反射层)
- 故障隔离: 插件失败不影响 log.query,可用 log.query 查看错误日志
- 易于扩展: 新增命令只需要 2 步 (创建 Handler + 创建 Plugin)
...
### 1.2 插件化架构已完成 (2026-02-01)

系统已成功重构为插件化架构,实现了以下核心目标:

**目标 1: 插件化架构** ✅

- 将命令处理改为插件系统,通过接口和特性自动发现
- 新增命令时只需要创建 Handler 和 Plugin 类,无需修改核心代码
- 核心命令 (如 `log.query`) 与项目专用命令 (如 K3) 完全解耦

**目标 2: 故障隔离** ✅

- 采用两层隔离架构,确保核心命令永不依赖反射
- 插件加载失败不影响其他插件和核心功能
- 开发新功能报错时,仍可使用 `log.query` 查看错误日志

**目标 3: 可移植性** ✅

- 删除 `K3Prefab/` 文件夹后项目可正常编译运行
- 通过配置文件控制启用/禁用哪些插件模块
- 项目专用功能可以作为独立模块移植到其他项目

**目标 4: 能力管理便捷性** ✅

- 新增命令只需要 2 步: 创建 Handler + 创建 Plugin
- 删除命令只需要删除对应文件夹
- 保持对外调用协议不变 (pending/results/done 机制不变)
...
├── AgentCommandsConfig.cs             # 配置常量
└── AgentCommandsPlugin.cs             # 主入口

Assets/AgentCommands/                  # 工作目录(数据目录)
├── pending/                           # 输入队列
...
**核心理念**: 只有日志命令 (log.query) 属于内置层,硬编码注册不依赖反射。其他所有命令 (包括 Unity 通用的 prefab.\*) 都通过反射插件机制加载。

**⚠️ 重要说明**:

- **只有 log.query 是内置命令**,硬编码注册在 `CoreCommandsLoader` 中,Priority 0,永不依赖反射
- **不要其他内置命令** (如 system.status, plugin.list),避免增加维护成本
...
**工作目录(数据目录):** `Assets/AgentCommands/`
...
- 通过 `CommandHandlerRegistry` 统一管理所有命令处理器

### 6.2 插件化扩展方式 (已完成)

**新增命令的方式 (已简化):**

新增命令只需要 2 步,无需修改核心文件:

**步骤 1**: 创建 Handler

```csharp
// Plugins/Asset/Handlers/AssetQueryHandler.cs
using AgentCommands.Core;
using LitJson2_utf;

namespace AgentCommands.Plugins.Asset
{
    public class AssetQueryHandler
    {
        public const string CommandType = "asset.query";
...
```csharp
// Plugins/Asset/AssetCommandsPlugin.cs
using AgentCommands.Core;

namespace AgentCommands.Plugins.Asset
{
    [CommandPlugin("Asset", Priority = 30)]
    public class AssetCommandsPlugin : ICommandPlugin
    {
        public string Name => "Unity 资产查询命令";

        public void RegisterHandlers(CommandHandlerRegistry registry)
        {
            registry.Register("asset.query", AssetQueryHandler.Execute);
        }
...
- `CommandHandlerRegistry` 不再直接引用 `AgentCommands.K3Prefab.Handlers`
- 所有项目专用引用通过接口解耦
...
- [x] 创建 `ICommandPlugin` 接口,包含 Name, RegisterHandlers, Initialize, Shutdown 方法
- [x] 创建 `[CommandPlugin]` 特性,支持 Name 和 Priority 参数
- [x] 创建 `CommandPluginLoader` 类,实现反射扫描和插件加载
- [x] 创建 `CoreCommandsLoader` 类,硬编码注册核心命令 (仅 log.query)
- [x] 重构 `CommandHandlerRegistry` 类,移除硬编码的 Handler 引用

### 8.2 插件实现验收

- [x] 创建 `LogCommandsPlugin` 类,但 RegisterHandlers() 为空实现 (log.query 已在 CoreCommandsLoader 中硬编码)
- [x] 创建 `PrefabCommandsPlugin` 类,注册所有 `prefab.*` 命令
- [x] 创建 `K3PrefabPlugin` 类,注册所有 `k3prefab.*` 命令

### 8.3 故障隔离验收

- [x] 插件加载失败时,不影响其他插件加载
- [x] 插件加载失败时,核心命令仍可用
- [x] 核心命令加载失败时,框架进入"不可用状态"
- [x] 所有加载失败信息写入 `plugin_errors.json` (如实现)

### 8.4 能力管理验收

- [x] 新增命令只需要创建 Handler + Plugin,无需修改核心文件
- [x] 删除 `K3Prefab/` 文件夹后项目可正常编译运行
- [x] 删除 `K3Prefab/` 文件夹后,k3prefab.\* 命令不存在

### 8.5 功能回归验收 (原框架功能)

- [x] 删除 `Assets/AgentCommands/` 后重新打开 Unity,目录会自动创建完整.
- [x] 往 `pending/` 放入 1 个合法批量命令 json,json 会被处理,`results/` 先出现 processing 再变 completed,命令文件被移动到 `done/`.
- [x] 批量命令执行后,`results` 数组顺序与输入 `commands` 数组顺序完全一致.
- [x] 批量命令的批次级别统计字段正确: `totalCommands`,`successCount`,`failedCount`.
- [x] 部分成功模式: 某个命令失败时,后续命令继续执行,最终返回所有结果.
- [x] 命令级别 `timeout` 优先级高于批次级别 `timeout`.
- [x] 命令超时时,该命令失败,但继续执行后续命令.
- [x] 批次超时时,立即中止执行,返回已完成命令的结果,未执行命令标记为 `SKIPPED` 错误.
- [x] 命令缺少必填字段时,该命令失败,不影响后续命令.
- [x] 未知命令类型时,该命令失败,错误码 `UNKNOWN_TYPE`.
- [x] 正则表达式非法时,该命令失败,错误码 `INVALID_REGEX`.
- [x] `commands` 数组为空时,批次级别错误,错误码 `INVALID_FIELDS`.
- [x] `batchId` 字段缺失时,批次级别错误,错误码 `INVALID_FIELDS`.
- [x] 单个命令也必须使用批量格式(包含 `commands` 数组).
- [x] 批量执行多个命令时,严格按输入顺序串行执行,不并行.
- [x] 在处理过程中触发域重载(例如修改脚本触发重编译),插件重启后能继续处理并最终归档.
- [x] 连续放入 21 个合法批量命令并等待全部处理完成,当第 21 个命令完成后,最老的 1 个 results 和对应的 done 同名归档会被自动删除(results 中始终保留最新的 20 条最终结果).
- [x] 在处理过程中检查 `results/` 目录,即使已有 20 条或更多结果,`processing` 状态的结果文件也不会被清理;只有 `completed` 的最终结果才参与计数和清理.

### 8.6 可移植性验收

- [x] 移除 `K3Prefab/` 文件夹后,项目编译无错误
- [x] 移除 `K3Prefab/` 文件夹后,Unity 启动无错误
- [x] 移除 `K3Prefab/` 文件夹后,其他命令功能正常
...
### 9.1 与 98\_插件化重构文档的关系

本文档 (`01_整体与框架需求.md`) 已融合 `98_插件化重构与能力管理.md` 的核心内容, 反映了插件化架构完成后的最新状态。

**保留内容**:

- 原有的命令文件协议 (pending/results/done 机制)
- 原有的批量命令处理流程
- 原有的超时和错误处理机制
- 原有的所有示例和验收清单
...
- 2.1 插件化目录结构 (详细文件树)
- 2.2 两层隔离架构 (内置层 + 反射层)
- 6. 模块化/可扩展性要求 (完全重写,反映插件化实现)
  - 6.1 命令分发机制
  - 6.2 插件化扩展方式 (含完整代码示例)
  - 6.3 插件接口定义
  - 6.4 故障隔离机制
  - 6.5 可移植性支持
- 8. 验收清单 (重新组织,增加插件化相关验收项)
  - 8.1 核心框架验收
  - 8.2 插件实现验收
  - 8.3 故障隔离验收
  - 8.4 能力管理验收
...
1. **初期** (01 文档 v1): 实现基础框架,只支持 log.query
2. **扩展期** (01 文档 v2): 增加 prefab._ 和 k3prefab._ 命令,硬编码注册
3. **重构期** (98 文档): 实现插件化架构,故障隔离,可移植性
4. **简化期** (98 文档 v2): 删除 system.status 和 plugin.list,只保留 log.query 作为内置命令
5. **当前** (01 文档 v3): 融合 98 文档,反映最新架构状态

### 9.3 相关文档

- `98_插件化重构与能力管理.md` - 插件化重构详细设计 (已完成,保留作参考)
- `02_日志功能需求_log.query.md` - log.query 命令详细需求
- 其他命令需求文档 (如 prefab._, k3prefab._)
