    ...
    internal static class LogCache
    {
        ...
        private static void HandleLog(string logString, string stackTrace, LogType type)
        {
            lock (_lock)
            {
                ...
                {
                    time = UnityAgentSkillsConfig.FormatTimestamp(DateTime.Now),
                    level = MapLevel(type),
                    message = logString ?? "",
                    stack = stackTrace ?? ""
                });
        ...
        /// 将输入 level 归一化为协议等级.
        /// </summary>
        /// <param name="raw">输入等级(允许大小写/空白变体).</param>
        /// <returns>标准等级("Log"/"Warning"/"Error"),未知则返回 null.</returns>
        private static string NormalizeLevel(string raw)
        {
            ...
            if (string.Equals(trimmed, "Log", StringComparison.OrdinalIgnoreCase)) return "Log";
            if (string.Equals(trimmed, "Warning", StringComparison.OrdinalIgnoreCase)) return "Warning";
            if (string.Equals(trimmed, "Error", StringComparison.OrdinalIgnoreCase)) return "Error";
        ...
        public static List<LogEntry> Query(int n, string level, string[] keywords, string matchMode, bool includeStack)
        {
            if (n < 0) n = 0;
            ...
            lock (_lock)
            {
                ...
                {
                    seq = FilterByLevel(seq, normalizedLevel);
                }

                if (keywords != null && keywords.Length > 0)
                {
                    seq = FilterByKeyword(seq, keywords, matchMode ?? "Fuzzy", includeStack);
                }
            }
        ...
        /// <param name="includeStack">是否包含堆栈.</param>
        /// <returns>过滤后的序列.</returns>
        private static IEnumerable<LogEntry> FilterByKeyword(IEnumerable<LogEntry> seq, string[] keywords, string matchMode, bool includeStack)
        {
            if (keywords == null || keywords.Length == 0)
            {
            ...
            if (matchMode == "Regex")
            {
                Regex regex = new Regex(keywords[0]);
                foreach (var e in seq)
                {
