...
namespace UnityAgentSkills.Core
{
    ...
    internal static class CommandErrorFactory
    {
        ...
        public static CommandError CreateInvalidRegexError(string detail)
        {
            ...
            {
                code = UnityAgentSkillCommandErrorCodes.InvalidRegex,
        ...
        public static CommandError CreateUnknownCommandError(string commandType, string detail)
        {
            return new CommandError
            {
                code = UnityAgentSkillCommandErrorCodes.UnknownType,
        ...
        {
            // `INVALID_FIELDS` 的 message 需要面向非专业人士可读.
            // 因此当业务侧通过异常前缀透传了可读文本时,优先使用该文本作为 message.
            string safeDetail = detail ?? string.Empty;
            string userMessage = string.IsNullOrWhiteSpace(safeDetail) ? MessageInvalidFields : safeDetail;

            return new CommandError
            {
                code = UnityAgentSkillCommandErrorCodes.InvalidFields,
                message = userMessage,
                detail = safeDetail
            };
        }
        ...
        public static CommandError CreatePrefabNotFoundError(string prefabPath)
        {
            return new CommandError
            {
                code = UnityAgentSkillCommandErrorCodes.RuntimeError,
        ...
        public static CommandError CreateSkippedError(int batchTimeoutMs)
        {
            return new CommandError
            {
                code = UnityAgentSkillCommandErrorCodes.Skipped,
