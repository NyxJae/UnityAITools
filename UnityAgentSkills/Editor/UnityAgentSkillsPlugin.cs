...
namespace UnityAgentSkills
{
    ...
    internal static class UnityAgentSkillsPlugin
    {
        ...
        {
            Directory.CreateDirectory(UnityAgentSkillsConfig.CommandsRootAbsolutePath);
        ...
        private sealed class BatchExecutionSession
        {
            ...
            private UnityAgentSkills.Plugins.Log.Handlers.LogRefreshCommandHandler.RefreshJob _refreshJob;
            private bool _refreshCompilationObserved;
            ...
            public void Tick()
            {
                ...
                try
                {
                    ...
                    if (string.Equals(cmd.type, UnityAgentSkills.Plugins.Log.Handlers.LogRefreshCommandHandler.CommandType, StringComparison.OrdinalIgnoreCase))
                    {
                        DateTime now = DateTime.Now;
                        if (!_waitingRefresh)
                        {
                            _refreshJob = UnityAgentSkills.Plugins.Log.Handlers.LogRefreshCommandHandler.CreateJob(cmd.@params);
                            _refreshCompilationObserved = EditorApplication.isCompiling;
                            UnityAgentSkills.Plugins.Log.Handlers.LogRefreshCommandHandler.BeginRefresh(_refreshJob);
                            _refreshCompilationObserved = _refreshCompilationObserved || EditorApplication.isCompiling;
                        ...
                        JsonData refreshResult;
                        CommandError refreshError;
                        bool refreshCompleted = UnityAgentSkills.Plugins.Log.Handlers.LogRefreshCommandHandler.TryComplete(
                            _refreshJob,
                            now,
                            _currentCmdStartTime,
                            _currentCmdTimeoutMs,
                            _refreshCompilationObserved,
                            out refreshResult,
                            out refreshError);
                        ...
                        else
                        {
                            cmdResult.status = UnityAgentSkillCommandStatuses.Success;
                            cmdResult.finishedAt = UnityAgentSkillsConfig.FormatTimestamp(now);
                            cmdResult.result = BuildUnifiedResultPayload(cmd, cmdResult.status, refreshResult, null);
                            _batchResult.successCount++;
                        }
            ...
            private static bool IsKnownCommandErrorCode(string code)
            {
                switch (code)
                {
                    ...
                    case UnityAgentSkillCommandErrorCodes.MenuItemNotFound:
                    case UnityAgentSkillCommandErrorCodes.MenuItemNotExecutable:
                    case UnityAgentSkillCommandErrorCodes.ForbiddenEditorAction:
                        return true;
                    default:
                        return false;
                }
