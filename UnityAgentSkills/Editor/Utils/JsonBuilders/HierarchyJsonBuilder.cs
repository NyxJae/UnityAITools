...
namespace UnityAgentSkills.Utils.JsonBuilders
{
    ...
    internal static class HierarchyJsonBuilder
    {
        ...
        /// <summary>
        /// 构建搜索模式(matches)的结果JSON.
        /// 当 prefab.queryHierarchy 传入有效 `nameContains:string[]` 搜索条件时使用.
        /// </summary>
        /// <param name="prefabPath">预制体路径(以 "Assets/" 开头).</param>
        /// <param name="prefab">预制体根 GameObject.</param>
        /// <param name="matches">匹配到的扁平节点列表(最多 maxResults 条).</param>
        /// <param name="totalMatches">总命中数(未截断).</param>
        /// <returns>搜索模式(matches)的结果 JSON.</returns>
            ...
            int totalMatches)
        {
            ...
            result["matches"] = BuildMatchesArray(matches, prefabPath, prefab);

            return result;
        }

        private static JsonData BuildMatchesArray(System.Collections.Generic.List<HierarchyNode> matches, string prefabPath, GameObject prefab)
        {
            JsonData arr = JsonResultBuilder.CreateArray();
            if (matches == null)
            {
            ...
            string normalizedPrefabPath = prefabPath ?? "";
            string rootName = prefab != null ? prefab.name : "";
            foreach (var node in matches)
            {
                ...
                json["depth"] = node.depth;
                json["isActive"] = node.isActive;
                json["prefabPath"] = normalizedPrefabPath;
                json["rootName"] = rootName;
