...
{
    /// <summary>
    /// scene.batchEdit 命令处理器.
    /// 严格事务: 单次场景编辑会话内串行执行子操作,仅当全部成功时保存一次.
    /// </summary>
    internal static class SceneBatchEditHandler
    {
        /// <summary>
        /// 支持的命令类型.
        /// </summary>
        public const string CommandType = "scene.batchEdit";
        ...
        public static JsonData Execute(JsonData rawParams)
        {
            ...
            string sceneName = SceneEditSession.ValidateSceneNameOrThrow(parameters.GetString("sceneName", null));
            ...
            using (SceneEditSession session = new SceneEditSession(sceneName))
            {
                for (int i = 0; i < operations.Count; i++)
                {
                    ...
                    try
                    {
                        ...
                        if (input.@params != null && input.@params.IsObject && input.@params.ContainsKey("sceneName"))
                        {
                            throw new ArgumentException(UnityAgentSkillCommandErrorCodes.InvalidFields + ": operations[" + i + "].params must not contain sceneName");
                        }

                        JsonData opResult = ExecuteOperation(session, input);
        ...
        private static JsonData ExecuteOperation(SceneEditSession session, OperationInput input)
        {
            if (input == null)
            {
            ...
            return SceneBatchEditOperationExecutor.Execute(session, input.type, input.@params);
        }
    }
}
