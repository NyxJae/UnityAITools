...
namespace UnityAgentSkills.Plugins.PlayMode.Utils
{
    ...
    internal static class PlayModeUIQueryUtils
    {
        ...
        public static JsonData QueryAll(string nameContains, string textContains, string[] componentFilter, bool visibleOnly, bool interactableOnly, int maxResults, JsonData screenRect)
        {
            ...
            string normalizedNameContains = string.IsNullOrWhiteSpace(nameContains) ? string.Empty : nameContains.Trim();
            string normalizedTextContains = string.IsNullOrWhiteSpace(textContains) ? string.Empty : textContains.Trim();
            ...
            for (int i = 0; i < selectables.Length; i++)
            {
                ...
                if (!string.IsNullOrEmpty(normalizedNameContains) && target.name.IndexOf(normalizedNameContains, StringComparison.OrdinalIgnoreCase) < 0)
                {
                    continue;
                }

                string resolvedText = ResolveText(target);
                if (!string.IsNullOrEmpty(normalizedTextContains) && resolvedText.IndexOf(normalizedTextContains, StringComparison.OrdinalIgnoreCase) < 0)
                {
                    continue;
                }
