    ...
    internal static class K3PrefabSetComponentPropertiesHandler
    {
        ...
        public static JsonData Execute(JsonData rawParams)
        {
            ...
            if (parameters.Has("k3Id"))
            {
                ...
                if (k3IdInt < 0)
                {
                    throw new ArgumentException(UnityAgentSkillCommandErrorCodes.InvalidFields + ": k3Id must be non-negative");
                }
                k3Id = (uint)k3IdInt;
            }
            if (k3Id == 0)
            {
                throw new ArgumentException(UnityAgentSkillCommandErrorCodes.InvalidFields + ": k3Id must be greater than 0");
            }

            int index = parameters.GetInt("index", 0);
            if (index < 0)
            {
                throw new ArgumentException(UnityAgentSkillCommandErrorCodes.InvalidFields + ": index must be >= 0");
            }
