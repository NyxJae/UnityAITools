    ...
    internal static class SceneQueryComponentsHandler
    {
        ...
        public static JsonData Execute(JsonData rawParams)
        {
            ...
            if (parameters.Has("componentFilter"))
            {
                ...
                {
                    componentFilter = new string[componentFilterData.Count];
                    for (int i = 0; i < componentFilterData.Count; i++)
                    {
                        componentFilter[i] = componentFilterData[i].ToString();
                    }
                }
                else
                {
                    throw new ArgumentException(
                        UnityAgentSkillCommandErrorCodes.InvalidFields + ": componentFilter must be an array of strings");
                }
            }

            bool includePrivateFields = parameters.GetBool("includePrivateFields", false);

            var components = ComponentPropertyReader.ReadComponents(target, componentFilter, includePrivateFields);
