...
- ✅ 支持多个 Unity 项目同时接入（使用项目路径作为唯一标识）
...
```bash
# 只查询特定 Unity 项目的日志（使用项目路径片段）
node query_unity_logs.js --count 20 --client "MyProject"

# 多个项目同时接入时很有用（支持路径模糊匹配）
node query_unity_logs.js --count 20 --client "ProjectA"

# 使用完整路径精确匹配
node query_unity_logs.js --count 20 --client "E:/Project/GameProject"
```
...
4. 多项目环境：
   - 不指定 --client 时，自动返回最近活跃项目的日志
   - 使用 --client 筛选特定项目（支持路径片段或完整路径）
5. 显示堆栈：添加 --stack 参数（如：--count 20 --fuzzy "Error" --stack）
```
...
- 显示堆栈：`node query_unity_logs.js --count 20 --fuzzy "Error" --stack`
...
- 不指定 --client 时，自动返回最近活跃项目的日志
- 使用 --client 参数可筛选特定项目（支持路径片段或完整路径）
```
...
- node query_unity_logs.js --count 20 --client "ProjectName"（特定项目）
- node query_unity_logs.js --count 20 --fuzzy "Error" --stack（显示堆栈）

多项目开发提示：

- 不指定 --client 时，自动返回最近活跃项目的日志
- 查询结果会显示 Active Client Path
- 使用 --client 参数可筛选特定项目（支持路径片段或完整路径）
```
...
| `--stack`   | 显示堆栈信息             | -     | 添加此参数即显示堆栈      |

**组合规则：**

- `--count` 和 `--minutes` 不能同时使用
- `--keyword`、`--fuzzy`、`--regex` 不能同时使用
- 可以组合：数量/时间 + 关键词匹配 + 客户端筛选 + 堆栈显示
- `--client` 和 `--stack` 可以与任何参数组合使用

**示例：**

- ✅ `--count 20 --fuzzy "error" --client "ProjectA"`
- ✅ `--minutes 5 --regex "Error.*player" --stack`
- ✅ `--count 10 --client "GameProject" --stack`
...
**症状：**
查询时显示的是其他 Unity 项目的日志，而不是当前项目的

**原因：**
系统默认返回"最近活跃项目"（最后发送日志的 Unity 客户端）的日志

**解决方案：**

1. 查看查询结果中显示的 `Active Client Path`
2. 使用 `--client` 参数指定正确的项目路径：
...
**症状：**
查询结果中堆栈信息（Stack）占用了大量空间

**解决方案：**

默认情况下不显示堆栈信息。如需查看堆栈，使用 `--stack` 参数：

```bash
# 显示堆栈信息
node query_unity_logs.js --count 20 --fuzzy "Error" --stack
```
...
- 不指定 `--client` 时，自动返回最近活跃项目的日志（最后发送日志的客户端）
- 查询结果会显示 `Active Client Path`，帮助你确认查询的是哪个项目
- 使用项目路径片段或完整路径可以精确筛选特定项目

```bash
# 查询最近活跃项目的日志（推荐日常使用）
node query_unity_logs.js --count 50

# 只查询 ProjectA 的日志（使用路径片段）
node query_unity_logs.js --count 20 --client "ProjectA"

# 只查询 ProjectB 的错误日志（使用路径片段）
node query_unity_logs.js --minutes 10 --fuzzy "Error" --client "ProjectB"

# 使用完整路径精确匹配（推荐项目名重复时使用）
node query_unity_logs.js --count 20 --client "E:/Project/UnityGame/ProjectA"
```
...
- 多个项目会自动使用项目路径作为唯一标识，查询时可按路径筛选
- Console 清空操作会同步到服务端
- 建议将查询工具路径加入 AI 工具配置，方便快速调用
- 不指定 `--client` 时，自动返回最近活跃项目的日志
- 默认不显示堆栈信息，使用 `--stack` 参数可显示完整堆栈
