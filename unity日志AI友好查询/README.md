...
```bash
node unity日志AI友好查询/unity_log_server.js
```
...
```bash
# Windows
start /B node unity日志AI友好查询/unity_log_server.js

# Linux/Mac
nohup node unity日志AI友好查询/unity_log_server.js > /dev/null 2>&1 &
```
...
1. 确保日志服务已启动：node unity日志AI友好查询/unity_log_server.js
...
工具位置：
- 服务端：unity日志AI友好查询/unity_log_server.js
- 查询工具：unity日志AI友好查询/query_unity_logs.js
...
常用查询：
- node query_unity_logs.js --count 20（最近20条）
- node query_unity_logs.js --minutes 5（最近5分钟）
...
1. **启动服务**：`node unity日志AI友好查询/unity_log_server.js`
2. **配置 Unity**：将 `UnityLogClient.cs` 放到 `Assets/Editor/`
3. **打开 Unity**：插件自动连接
